...
<template>
  <div class="absolute top-3 left-1/2 -translate-x-1/2 z-20 w-72 bg-black/70 backdrop-blur-2xl p-4 rounded-2xl border border-white/10 shadow-2xl transition-all duration-300">
    
    <!-- Header / Toggle -->
    <div class="flex justify-between items-center -mt-2 mb-3">
      <span class="text-[10px] text-white/60 font-black tracking-[0.2em] uppercase">Sim Controls</span>
      <button 
        @click="emit('update:isCollapsed', !isCollapsed)"
        class="text-white/40 hover:text-white transition-colors p-1"
      >
        <svg v-if="isCollapsed" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
        <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m18 15-6-6-6 6"/></svg>
      </button>
    </div>
    ...
    <div v-if="!isCollapsed" class="flex flex-col gap-4 mt-6 pt-4 border-t border-white/10">
      ...
      <div class="flex flex-col gap-2">
        <div class="flex justify-between items-center">
          <span class="text-[9px] text-white/40 font-bold uppercase tracking-widest">Ecliptic Longitude Δ</span>
      ...
      <div class="flex flex-col gap-2">
        <div class="flex justify-between items-center">
          <span class="text-[9px] text-white/40 font-bold uppercase tracking-widest">Ecliptic Latitude Δ</span>
          <span class="text-white font-mono text-xs">{{ lat.toFixed(1) }}°</span>
        </div>
        <input
          :value="lat"
          @input="emit('update:lat', Number($event.target.value))"
          type="range" min="-5.2" max="5.2" step="0.1"
          class="custom-slider"
        />
      </div>
      ...
      <div class="flex flex-col gap-2">
        <div class="flex justify-between items-center">
          <span class="text-[9px] text-white/40 font-bold uppercase tracking-widest">Ecliptic Tilt</span>
          <span class="text-white font-mono text-xs">{{ tilt }}°</span>
        </div>
        <input
          :value="tilt"
          @input="emit('update:tilt', Number($event.target.value))"
          type="range" min="15" max="165" step="1"
          class="custom-slider"
        />
      </div>
      ...
      <div class="flex items-center justify-between pt-4 mt-2 border-t border-white/10">
        <span class="text-[9px] text-white/40 font-bold uppercase tracking-widest">Guiding Aids</span>
        <button

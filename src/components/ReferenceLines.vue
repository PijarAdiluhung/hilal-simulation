...
const labelTransforms = computed(() => ({
  long: `translate(${projectedPoint.value.x}, ${projectedPoint.value.y})`,

  lat: `translate(
    calc((${props.sunPos.left} + ${projectedPoint.value.x}) / 2),
    calc((${props.sunPos.top} + ${projectedPoint.value.y}) / 2)
  )`,

  elong: `translate(
    calc((${props.sunPos.left} + ${props.moonPos.left}) / 2),
    calc((${props.sunPos.top} + ${props.moonPos.top}) / 2)
  )`,
}))
</script>
...
<template>
  <svg class="absolute inset-0 w-full h-full pointer-events-none z-0">
    ...
    <g v-if="props.sunPos && props.moonPos" class="fill-white text-[12px] font-bold select-none">
      <!-- Longitude -->
      <text class="fill-sky-400" :style="{ transform: labelTransforms.long }" dx="10" dy="-10">
      ...
      <text
        class="fill-white/80"
        :style="{ transform: labelTransforms.lat }"
        dy="-10"
        text-anchor="middle"
      >
      ...
      <text
        ...
        :style="{ transform: labelTransforms.elong }"
        dx="-10"
        dy="-10"
        text-anchor="end"
      >
